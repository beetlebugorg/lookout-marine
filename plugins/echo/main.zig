...
//! timer, echoes TCP bytes into the log, draws one overlay symbol, and tries
//! to raise an alert it was deliberately NOT granted.
//!
...
fn draw() void {
    ...
    ov.symbol("echo", .target, lon, lat, 0, .target, 1.0);
    _ = ov.send();
}
