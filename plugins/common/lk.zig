...
/// Publish one frame on the bus. The topic must be in the manifest's
/// `bus.publish` list; frames over 64 KiB are refused. Returns the number of
/// plugins reached (never this one); zero readers is not an error.
...
pub const Event = union(enum) {
    ...
    /// A frame another plugin published on a topic in the manifest's
    /// `bus.read` grant. The payload format is the topic's contract, not the
    /// host's.
    bus_data: BusData,
    ws_open: WsOpen,
    ws_data: WsData,
...
/// Split a `u32 json_len | head | body` payload, the framing HTTP_RESPONSE
/// and BUS_DATA share. Null when the length prefix overruns the payload.
...
/// Parse a BUS_DATA payload: `u32 json_len | {"topic":…,"from":…} | raw
/// bytes`, the same envelope HTTP_RESPONSE uses. A malformed payload parses
/// as an empty topic, which no handler matches.
fn parseBusData(payload: []const u8) BusData {
    ...
    return .{
        ...
        // A missing `from` drops the whole frame: handlers use it to decide
