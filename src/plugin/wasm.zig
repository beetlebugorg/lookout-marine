...
pub fn initRuntime() Error!void {
    ...
    //
    // Every module is bytecode. The runtime is built with WAMR_BUILD_AOT=0,
    // so wasm_runtime_load accepts bytecode only. See scripts/build-wamr.sh
    // for why ahead-of-time compilation is not viable on these targets.
