...
pub const Manifest = struct {
    ...
    udp_ports: []u16 = &.{},
    /// The topics `bus.publish` and `bus.read` named, same rule. The host
    /// checks only a topic name's shape, never its meaning, so a manifest may
    /// name a topic nothing else uses yet.
    pub_topics: [][]u8 = &.{},
    sub_topics: [][]u8 = &.{},
...
/// The topics a `{"bus.publish":[…]}` or `{"bus.read":[…]}` entry names. Any
/// name of the right shape is accepted (lowercase letters, digits, `.`, `_`,
/// and `-`), so a new topic needs no host change. An empty list refuses the
/// manifest, like an empty host list.
...
test "a bus grant names its topics, any topic of the right shape" {
    const a = t.allocator;
    // `to-be-invented` is a topic no plugin uses yet; it parses like any
    // other well-shaped name.
    var m = try parseManifest(a,
    ...
    const bad = [_][]const u8{
        // A bare or empty grant is over-broad, refused like the net caps.
        "{\"id\":\"x\",\"api\":1,\"capabilities\":[\"bus.publish\"]}",
        "{\"id\":\"x\",\"api\":1,\"capabilities\":[\"bus.read\"]}",
        "{\"id\":\"x\",\"api\":1,\"capabilities\":[{\"bus.read\":[]}]}",
        // Lowercase only, at most 32 bytes, no spaces, no duplicates,
        // strings only.
