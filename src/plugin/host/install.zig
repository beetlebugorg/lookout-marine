...
/// Print a topic list as the manifest spells it. Separate from
/// `writeHostList` because `local` is a reserved word there but an ordinary
/// topic name here: the host writer would render a topic named `local` as
/// "your own network".
...
test "the consent sentences read exactly as install.md words them" {
    ...
    const expect = [_][]const u8{
        ...
        // `local` is an ordinary topic here but a reserved word in the net
        // grants below.
