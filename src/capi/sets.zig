...
pub const lookout_sets = sets.Sets;
pub const lookout_set = sets.Set;
...
/// lookout-library.h.
export fn lookout_sets_open(store: ?*settings.Store) ?*lookout_sets {
    const s = store orelse return null;
    return sets.Sets.open(gpa, capi_io, s) catch null;
}

export fn lookout_sets_close(s: ?*lookout_sets) void {
    if (s) |x| x.close();
}

/// 1 since the last poll, then clears. A background scan landing raises it.
export fn lookout_sets_changed(s: ?*lookout_sets) c_int {
...
export fn lookout_sets_all(s: ?*lookout_sets, out_n: ?*usize) ?[*]const *const lookout_set {
    const x = s orelse {
        count(out_n, 0);
        return null;
    };
...
export fn lookout_sets_add(s: ?*lookout_sets, path: ?[*:0]const u8) c_int {
    const x = s orelse return 0;
    return @intFromBool(x.add(span(path)));
}

export fn lookout_sets_remove(s: ?*lookout_sets, path: ?[*:0]const u8) c_int {
    const x = s orelse return 0;
    return @intFromBool(x.remove(span(path)));
}

export fn lookout_sets_set_on(s: ?*lookout_sets, path: ?[*:0]const u8, on: c_int) c_int {
    const x = s orelse return 0;
    return @intFromBool(x.setOn(span(path), on != 0));
}

export fn lookout_sets_is_on(s: ?*lookout_sets, path: ?[*:0]const u8) c_int {
    const x = s orelse return 0;
    return @intFromBool(x.isOn(span(path)));
}

/// The charts to open. Borrowed until the next call that changes the list.
export fn lookout_sets_compose(s: ?*lookout_sets, out_n: ?*usize) ?[*]const [*:0]const u8 {
    const x = s orelse {
        count(out_n, 0);
        return null;
    };
