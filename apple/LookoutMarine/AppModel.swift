...
final class AppModel: ObservableObject {
    ...
    /// the chart is built from this. The watch itself is shared with the other
    /// apps; this mirrors it so the views that observe the model keep working.
    @Published var alerts: [PluginAlert] = []

    private let alertWatch = AlertWatch()
    private var alertMirror: AnyCancellable?

    /// Start watching for alerts. Called once the plugin layer is up.
    func startAlertWatch() {
        guard let controller else { return }
        if alertMirror == nil { alertMirror = alertWatch.$alerts.assign(to: \.alerts, on: self) }
        alertWatch.start(host: controller)
    }
