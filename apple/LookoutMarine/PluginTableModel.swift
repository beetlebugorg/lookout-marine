//
//  What a plugin's declared table is, and how its values read.
//
...
//  already in the order they are to be shown. This file is the declaration,
//  the rows, and the formatting, and it knows nothing about what any plugin
//  does or what draws it.
//
//  UNITS ARE THE SHELL'S. The column type says what a number means: distance
//  is meters, speed meters per second, bearing degrees true, duration seconds,
...
/// What a table needs from the chart: the declarations, the rows, and the way
/// to tell a plugin its table is being read. Each app has its own chart
/// controller, and this is the whole of what a table asks of one.
@MainActor
protocol PluginTableHost: AnyObject {
    ...
    /// A plugin builds rows only while something is reading them.
