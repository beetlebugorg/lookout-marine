...
# apple/build-mac/Build/Products/Debug/LookoutMarine.app. One app path on disk
...
set -e
REPO="${0:A:h:h}"
OUT="${OUT:-$REPO/apple/build-mac/Build/Products/Debug}"
# Intermediates (repacked archives, the bare executable) stay out of the
# products directory, beside Xcode's own.
WORK="$REPO/apple/build-mac/Build/Intermediates.noindex/build-dev"
...
xcrun swiftc -swift-version 5 -sdk "$SDK" -target arm64-apple-macosx26.0 \
  ...
  -o "$WORK/LookoutMarine" apple/LookoutMarine/**/*.swift 2>"$WORK/swiftc.log" \
