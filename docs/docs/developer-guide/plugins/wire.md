...
| 18 | `BUS_DATA` | 0 | A frame another plugin published on a topic in your `bus.read` grant: `u32 json_len`, then `{"topic":"nmea0183","from":"org.beetlebug.nmea0183"}`, then the raw bytes (the same envelope as `HTTP_RESPONSE`). The payload format is the topic's contract, not the host's. |
...
| `bus_publish` | `(topic_ptr, topic_len, ptr, len) -> i32` | `bus.publish` + its topic list | Number of subscribers reached (never the publisher itself), or -1. Frames over 64 KiB are refused. There is no `bus_subscribe`: a `bus.read` grant is the subscription. |
...
| `bus.read` | receive `BUS_DATA` for the topics the grant names; revoking the grant stops delivery |
...
**Bus topics are an open vocabulary.** The host validates only a topic name's shape (1 to 8 per grant, each 1–32 bytes of lowercase letters, digits, `.`, `_`, and `-`, matched exactly), never its meaning, so two plugins can adopt a new topic without a host change and a manifest may name a topic nothing else uses yet. A topic's payload format is a contract between the plugins that use it. The bus is fire-and-forget: nothing is retained or replayed, and a full queue drops the frame for that plugin only, counted and logged. Keep state in the vessel store and datasets in storage; the bus carries events.
