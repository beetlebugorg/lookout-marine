...
/* Water and land are the core's DEPMD and LANDA in the app's scheme, so the
 * picker sits in the chart's own palette. The accent is pinned, as the compass
 * rose in ui/startup-view.c pins it: cairo has no way to ask CSS for
 * @accent_color. */
#define LK_TOKEN_ALPHA 0.55
...
typedef struct {
  ...
  LkAppModel    *model; /* not owned; for the scheme */
  const LkPanel *panel;
} LkPanelState;

/* One S-52 token in the model's scheme, at the map's alpha. */
static void
lk_panel_token (cairo_t *cr, const char *token, LkAppModel *model)
{
  float rgba[4] = { 0.5f, 0.5f, 0.5f, 1.0f };

  lookout_s52_color (token, (guint32) lk_app_model_get_scheme (model), rgba);
  cairo_set_source_rgba (cr, rgba[0], rgba[1], rgba[2], LK_TOKEN_ALPHA);
}
...
lk_panel_draw (GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer user_data)
{
  ...
  cairo_paint (cr);
  lk_panel_token (cr, "DEPMD", state->model);
  cairo_paint (cr);

  lk_panel_rings (cr, window, 1, width, height);
  lk_panel_token (cr, "LANDA", state->model);
  cairo_fill (cr);
...
static GtkWidget *
lk_panel_new (LkNoaa *noaa, LkAppModel *model, const LkPanel *panel)
{
...
static GtkWidget *
lk_inset_new (LkNoaa *noaa, LkAppModel *model, const LkPanel *panel, double width)
{
  GtkWidget *box = gtk_box_new (GTK_ORIENTATION_VERTICAL, 2);
  GtkWidget *area = lk_panel_new (noaa, model, panel);
...
GtkWidget *
lk_coverage_map_new (LkNoaa *noaa, LkAppModel *model)
{
  GtkWidget *overlay = gtk_overlay_new ();
  GtkWidget *main_area = lk_panel_new (noaa, model, &lk_panel_main);
  ...
  gtk_box_append (GTK_BOX (corners), lk_inset_new (noaa, model, &lk_panel_alaska, LK_INSET_WIDTH));
  gtk_box_append (GTK_BOX (corners), lk_inset_new (noaa, model, &lk_panel_hawaii,
                                                   LK_INSET_WIDTH * 0.54));
