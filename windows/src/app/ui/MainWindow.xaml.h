...
namespace winrt::LookoutMarine::implementation
{
    ...
    private:
        ...
        void FirstRunPoll();
        /* The facts the core's setup reads off the app. first_run owns the
         * setup handle: SetupNote hands it the facts, SetupAct applies an
         * action, and SetupShouldRun returns whether setup comes up. The
         * shell sets setup_nothing_to_draw where it settles on no chart. */
        bool setup_nothing_to_draw{ false };
        lookout_setup_facts SetupFacts();
        void SetupNote();
        ...
        /* One panel of that map: a lon/lat window, west to east and south to
         * north, and the regions whose `panel` it is, drawn as the water they
         * cover. Each region is one tappable path, so the map picks as well as
         * it shows. */
        struct CoverageWindow
        {
            double w, e, s, n;
        };
        Microsoft::UI::Xaml::Controls::Border FirstRunCoveragePanel(CoverageWindow const &win,
                                                                    int panel, double width,
