...
/* The same two lists as the raster library: the folders the mariner installed,
 * and the ones switched off (a set is switched off, not removed, when its
 * water is not today's water). */
...
lk_store_load_chartsets(int **on_out)
{
    ...
    char **paths = load_list(LOOKOUT_STORE_CHARTSETS, "paths");
    if (paths == NULL || on_out == NULL)
        return paths;

    char **off = load_list(LOOKOUT_STORE_CHARTSETS, "off");
    int n_off = count_list(off);
    int n = count_list(paths);
    int *on = (int *)calloc((size_t)n + 1, sizeof *on);
    if (on != NULL)
        for (int i = 0; i < n; i++)
            on[i] = !in_list((const char *const *)off, n_off, paths[i]);
    lk_store_free_recents(off);
    *on_out = on;
    return paths;
}
...
edit_chartsets(const char *path, int op, int arg)
{
    ...
    int *on = NULL;
    char **existing = lk_store_load_chartsets(&on);
    if (existing == NULL || on == NULL) {
        lk_store_free_rasters(existing, on);
        return;
    }
    int have = count_list(existing);

    const char **paths = (const char **)malloc((size_t)(have + 1) * sizeof *paths);
    const char **off = (const char **)malloc((size_t)(have + 1) * sizeof *off);
    if (paths == NULL || off == NULL) {
        free(paths);
        free(off);
        lk_store_free_rasters(existing, on);
    ...
    for (int i = 0; i < have; i++) {
        ...
        int drawn = (hit && op == 2) ? (arg != 0) : on[i];
        paths[n++] = existing[i];
        if (!drawn)
            off[n_off++] = existing[i];
    }
    /* Already installed, and its switch is the mariner's. */
    if (op == 0 && !found)
        paths[n++] = path;

    save_list(LOOKOUT_STORE_CHARTSETS, "paths", paths, n);
    save_list(LOOKOUT_STORE_CHARTSETS, "off", off, n_off);
    wrote();
    free(paths);
    free(off);
    lk_store_free_rasters(existing, on);
